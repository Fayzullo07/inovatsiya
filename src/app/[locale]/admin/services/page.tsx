"use client"
import { serviceDeleteAPI, servicesGetAPI } from "@/api/AdminRequest";
    ...
    const mutationDeleteNew = useMutation(
        {
            mutationFn: async (id: any) => {
                return serviceDeleteAPI({ id });
            },
            onSuccess: () => {
                queryClient.invalidateQueries(); // Ma'lumotlarni yangilash
                toast.error("Deleted Service!");
            },
            onError: () => {
                    ...
                    {data?.data.services.map((item: any, i: number) => (
                        <tr key={i}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap  text-sm font-medium">
                                <Link href={`/${locale}/admin/services/edit/${item._id}`} className="text-indigo-600 hover:text-indigo-900">Edit</Link>
